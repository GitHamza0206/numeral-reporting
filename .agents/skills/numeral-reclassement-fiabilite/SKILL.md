...
- Dans ce dépôt template : le **moteur de règles** optionnel vit en **Python** (`scripts/reclasse_471_healing.py` — à remplir par dossier). On ne duplique pas un moteur de reclassement en TypeScript sous `src/reports/`. Le livrable TS est `src/reports/v<V>/model.ts` (validé par `src/schemas/report.ts`).
...
|------|-----------------------------------|--------------------|-------------------|-------------|
| **V0** | `"v0" as "v0" \| "v1"` | `npm run reclasse-471 -- --mode v0` | **Toutes** les lignes 471 de l’extrait | Identique **balance export** (pas de healing dans les totaux) |
| **V1** | `"v1" as "v0" \| "v1"` | `npm run reclasse-471 -- --mode v1` | **Résidu** seulement (hors lignes haute confiance du script) | **Aligné** sur le JSON healed (lignes marquées « ajusté », détails sous postes) |
...
5. **Exécuter** `npm run reclasse-471 -- --mode v0` puis `--mode v1` ; vérifier counts, totaux bloquants, `healed_pnl` (JSON).
...
7. **Typecheck** : `npm run typecheck`. **Conservateur sur le score** : ne monter que si le montant non traité baisse réellement et les reclassements sont justifiés.
